...
from src.middleware import AuthMiddleWare, ResponseTimeLoggingMiddleware, SQLAlchemySessionMiddleware, session
...
def add_backup_source(request: AddBackupSourceRequest, db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
...
def list_backup_sources(db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
    
    statement = select(Source).where(Source.tenant_id == user_info.tenant_id)   
    all_backup_sources = db_session.exec(statement).all()

    return ApiResponse(message="Backups sources retrieved successfully", data= list(all_backup_sources))

@app.delete("/api/v1/backup-sources/delete")
def delete_backup_source(request: DeleteBackupSourceRequest, db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
...
def update_backup_source(request: UpdateBackupSourceRequest, db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
    statement = select(Source).where(
        Source.id == request.source_id,
        Source.tenant_id == user_info.tenant_id
    )
...
def add_backup_destination(request: AddBackupDestinationRequest, db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
...
def list_backup_destinations(db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
    ...
    return ApiResponse(message="Backup destinations retrieved successfully", data=list(all_backup_destinations))

@app.delete("/api/v1/backup-destinations/delete")
def delete_backup_destination(request: DeleteBackupDestinationRequest, db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
...
def update_backup_destination(request: UpdateBackupDestinationRequest, db_session: Session = Depends(get_db_session), user_info: UserInfo = Depends(get_user_info)):
    ...
    return ApiResponse(message="Backup destination updated successfully", data=destination)

# / =========== BACKUP DESTINATION ===========
# =========== PERFORMING BACKUPS ===========
from src.base import BaseBackupDestinationManager, BaseBackupManager
def create_backup(backup_manager: BaseBackupManager, backup_destination_manager: BaseBackupDestinationManager):
    backup_manager.create_backup()

    backup_destination_manager.upload_backup()
